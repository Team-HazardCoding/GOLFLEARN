...
public class Upload{
	...
	public void uploadFiles(HttpServletRequest request, String userId)  {
		...
		try {
			...
			for(Part part: parts) {
				...
				// part.getName()으로는 모든 name값을 가지고옴 user_id ~ user_cerf
				//따라서 아래의 조건 하나 더 필요함
				if(paramName.equals("user_profile") || paramName.equals("pro_certf") || paramName.equals("lsn_thumbnail")) {
					...
					if(paramName.equals("user_profile")) { // formData에 담긴 parameter값이 user_profile이면		
						String fileName = "Profile"; // Profile로 저장
						System.out.println(fileName);
						part.write(uploadDirectory.getAbsolutePath()+"\\"+fileName+fileExtension);

					} else if (paramName.equals("pro_certf")) { // formData에 담긴 parameter값이 pro_certf이면
						String fileName = "Certification"; // Certification으로 저장
						System.out.println(fileName);
						part.write(uploadDirectory.getAbsolutePath()+"\\"+fileName+fileExtension);
					} 
				}
	...
	public void uploadLsnFiles(HttpServletRequest request, int lsnNo)  {
		//디렉토리에 파일 생성
		try {
			request.setCharacterEncoding("UTF-8");
		} catch (UnsupportedEncodingException e1) {
		...
		String uploadPathName= "C:\\Users\\sitsu\\OneDrive\\바탕 화면\\SemiProject\\front\\src\\main\\webapp\\lsn_images"; // 파일 업로드 할 절대경로 지정 
		//	String uploadPathName= "D:\\Golflearn_lib\\user_images\\" + userId ; // 파일 업로드 할 절대경로 지정 
		...
		try {
			...
			for(Part part: parts) {
				...
				// part.getName()으로는 모든 name값을 가지고옴 user_id ~ user_cerf
				//따라서 아래의 조건 하나 더 필요함
				if(paramName.equals("lsn_thumbnail")) {
					// substring() 문자열 가르는 메서드
					String originFileName = part.getSubmittedFileName(); // 업로드한 파일 이름 그대로 가져오는 것
					String fileExtension = originFileName.substring(originFileName.lastIndexOf("."));
					System.out.println("파일 확장자 : " + fileExtension);
